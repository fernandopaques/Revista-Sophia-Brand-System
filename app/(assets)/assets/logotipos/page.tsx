...
import { Hexagon, UploadCloud } from 'lucide-react'
...
export default function LogotiposPage() {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    listAssetsByCategory('logotipos').then(items => {
      if (cancelled) return
      setAssets(items)
      setLoading(false)
      loadPreviews(items)
        ...
        <div style={{
          ...
          <div>
            <p style={{ fontFamily: 'var(--font-heading)', fontSize: '22px', margin: '0 0 6px', color: '#1B3A5F' }}>
              {search ? 'Nenhum resultado' : 'Nenhum logotipo ainda'}
            </p>
            <p style={{ fontFamily: 'var(--font-ui)', fontSize: '14px', margin: 0 }}>
              {search
                ? `Não encontramos arquivos para "${search}".`
                : 'Faça o upload do primeiro logotipo para começar.'}
            </p>
          </div>
          {!search && (
            <button
